...
## tem que funcionar igual. Apenas alterações no alias_list devem bastar.
...
# In the first column you should put the name that could be written by anyone to refer a states, including 
# acronyms. Remember that all names will be coverted to upper case and removed accent. 
      ...
      if(!is.null(alias_list)){
          ...
          if(all(states%in%alias_list[,1])==FALSE)
          stop(
            paste(
              "The following states \n\n",
            states[!states%in%alias_list[,1]], 
 ...
 ## delete_cache parameter. 
  ...
  ## reading the files as Spatial Polygon 
      ...
      for (i in 1:length(names)) {  
        ...
        states_polygons[i] <- list(get(names[i]))
        
        }
        
      for (i in 1:length(states_polygons)){
        slot(states_polygons[[i]], "ID") = names[[i]]
      }
  
      assign('states_polygons',
             SpatialPolygons(states_polygons),
             envir = .GlobalEnv)
  ...
  if(isFALSE(aggregate)){
    ...
    rm(states_polygons, envir = .GlobalEnv)
